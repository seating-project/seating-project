...
export default async function AllotmentPage({ params }) {
  ...
  console.log("ROOMS SINGLE", roomSingle);
  const rangesNeeded = rooms.ranges;
  console.log("RANGES NEEDED", rangesNeeded);

  // const roomsNeededOne = roomSingle.filter((item: String) => item == String(date))  ;
  const roomNeededOne = roomSingle[String(date)];
  console.log("ROOM NEEDED ONE", roomNeededOne);

  const rangesNeededOne = rangesNeeded[String(date)];
  console.log("RANGES NEEDED ONE", rangesNeededOne);
  ...
  const roomGOD = Object.entries(rdata[0].rooms).sort(([a], [b]) =>
    a.localeCompare(b)
  );
  ...
  return (
    <div className="bg-white w-screen">
      {Object.keys(roomNeededOne).map((room) => {
        ...
        return (
          <ClassAllotment
            ...
            room={room}
            roomArray={roomNeededOne[room]}
            rows={template.num_rows}
            columns={template.num_columns}
            rangesSingle={rangesNeededOne[room]}
      ...
      {/* <ClassAllotment
        key={`page-${timestamp}-${order++}`}
        room={"MT5"}
        roomArray={roomSingle["MT5"]}
        rows={template.num_rows}
        columns={template.num_columns}
        rangesSingle={rangesNeeded["MT5"]}
        // exam={tdata[0].template_exam_name}
        exam={exam.name}
        room_strength={template.room_strength}
        single_seater={template.single_seater}
